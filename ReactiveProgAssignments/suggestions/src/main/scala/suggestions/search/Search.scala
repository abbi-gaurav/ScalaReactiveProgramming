...
object Search {
  ...
  val restAdapter: RestAdapter = new RestAdapter.Builder().setServer("http://en.wikipedia.org").build()
  ...
  def wikipediaSuggestionRetrofit(term: String): Future[List[String]] = {
    async {
      println(s"calling service for term $term")
      val (cb, f) = callbackFuture[Array[AnyRef]]
      service.suggestions(term, cb)
      val result: Array[AnyRef] = await { f }
      println(s"suggestions response $result")
