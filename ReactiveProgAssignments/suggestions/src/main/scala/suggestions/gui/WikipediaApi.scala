...
trait WikipediaApi {
  ...
  implicit class StringObservableOps(obs: Observable[String]) {
    ...
    /** Given a stream of search terms, returns a stream of search terms with spaces replaced by underscores.
      ...
      */
    def sanitized: Observable[String] = obs map { input =>
      input.replaceAll(" ", "_")
    }
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def recovered2: Observable[Try[T]] = Observable { subscriber =>
      obs.subscribe(x => subscriber.onNext(Success(x)),
        err => {
          subscriber.onNext(Failure(err))
    ...
    def timedOut(totalSec: Long): Observable[T] = {
      val timer: Observable[Long] = Observable.timer(totalSec seconds)
      obs.takeUntil(timer)
    }
    ...
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = {
      (obs concatMap (x => requestMethod(x) recovered))
    }
