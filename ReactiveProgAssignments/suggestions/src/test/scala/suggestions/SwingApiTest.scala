...
class SwingApiTest extends FunSuite {
  ...
  test("SwingApi should emit button instances when button is clicked to the observable") {
    val button = new swingApi.Button
    val clicks = button.clicks

    val observed1 = mutable.Buffer[Button]()
    val observed2 = mutable.Buffer[Button]()
    val sub1 = clicks subscribe {
      observed1 += _
    }
    ...
    button.click()
    sub1.unsubscribe()
    //second subscription after the first 2 clicks
    ...
    assert(observed1 == Seq(button,button), observed1)
    assert(observed2 == Seq(button), observed2)
  }
